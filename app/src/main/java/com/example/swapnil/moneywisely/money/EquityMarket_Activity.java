...
public class EquityMarket_Activity extends AppCompatActivity {
    public static final String TAG = EquityMarket_Activity.class.getSimpleName();

    Toolbar toolbar;

    ListView list_market;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_equity_market);
        ...
        getSupportActionBar().setTitle("Equity Market");
        ...
        list_market = (ListView) findViewById(R.id.list_market);

        if (support.checkInternetConnectivity()) {
            new GetEquityMarket().execute();
        } else {
    ...
    private class GetEquityMarket extends AsyncTask<String, String, String> {
        ...
        protected void onPreExecute() {
            ...
            super.onPreExecute();
            pd = new ProgressDialog(EquityMarket_Activity.this);
        ...
        protected String doInBackground(final String... strings) {
            try {
                ...
                request = new Request.Builder().url(AppConstant.getEquityMarket).post(reqbody).build();
                    ...
                    public void onResponse(Call call, final Response response) throws IOException {
                        if (response != null) {
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        Log.e("Market Equity ->", "" + jsonObject);
                                        ...
                                        if (str_success.equals("1")) {
                                            ...
                                            for (int i = 0; i < jsondata.length(); i++) {
                                                ...
                                                str_title = object.getString(AppConstant.TAG_equitymarket_title);
                                                str_link = object.getString(AppConstant.TAG_equitymarket_link);
                                                str_image = object.getString(AppConstant.TAG_equitymarket_img);

                                                HashMap<String, String> market = new HashMap<String, String>();
                                                market.put(AppConstant.TAG_equitymarket_title, str_title);
                                                market.put(AppConstant.TAG_equitymarket_link, str_link);
                                                market.put(AppConstant.TAG_equitymarket_img, str_image);
                                                market.put(AppConstant.TAG_equitymarket_desc, object.getString(AppConstant.TAG_equitymarket_desc));
                                                marketList.add(market);
                                            }
                                            marketAdapter = new MarketAdapter(EquityMarket_Activity.this, marketList, "equity");
                            ...
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    final AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(EquityMarket_Activity.this);
